...
from .forms import AssessmentForm, ClassForm
...
def delete_assessment_view(request, assessment_id):
    ...
    if assessment:
        ...
        assessment.delete()
        return render(
            request, "main/simple_message.html", {
                'message':
                    'Deletado com sucesso',
                'url_back':
                    reverse('manage_assessment',
                            kwargs={'course_id': assessment.course_id})
            })
    else:
        return render(
            request, "main/simple_message.html", {
...
def alter_assessment_view(request, assessment_id):
    ...
    if request.method == "GET":
        ...
        form.fields['course'].queryset = Course.objects.filter(
            teacher=request.user.id, completed_course=False)
        ...
        return render(
            request, "main/simple_message.html", {
                'message':
                    'Deletado com sucesso',
                'url_back':
                    reverse('manage_class',
                            kwargs={'course_id': class_.course_id})
            })
    else:
        return render(
            request, "main/simple_message.html", {
...
def alter_class_view(request, class_id):
    class_ = Class.objects.filter(id=class_id)[0]
    if request.method == "GET":
        form = ClassForm(instance=class_)
        form.fields['course'].queryset = Course.objects.filter(
            teacher=request.user.id, completed_course=False)
        return render(request, "main/alter_class.html", {"form": form})
...
def create_class_view(request, course_id):
    ...
    if request.method == "GET":
        ...
        form = ClassForm(request.POST, initial={'course': course})
        if form.is_valid():
            user = form.save()
            # return HttpResponse('<h1>Salvo</h1>')
            return render(
                request, "main/simple_message.html", {
                    'message':
                        'Salvo com sucesso',
                    'url_back':
                        reverse('manage_class', kwargs={'course_id': course.id})
                })
        else:
            form.fields['course'].queryset = Course.objects.filter(
                teacher=request.user.id, completed_course=False)
